...
import com.hibernate.crud.operations.manager.RegimeManager;
...
public class RegimeMainListFormController {

    private Main mainApp;
    private ObservableList<Regime> dataList = FXCollections.observableArrayList();
    private Regime currentEntity;
    ...
    @FXML
    private TableView<Regime> mainTable;
    ...
    @FXML
    private TableColumn<Regime, String> col1;
    ...
    void btnDeleteOnClick(ActionEvent event) {
        ...
        ObservableList<Regime> selectedEntity = mainTable.getSelectionModel().getSelectedItems();
...
//            List<RegimeLoadInfo> entityList = getEntityListFromExcel(file);
//
...
//        System.gc();
    ...
    void showDicList() {
        ...
        //Binding model and view elements
            ...
            for (Field fld : Regime.class.getDeclaredFields()) {
...
//                } else {
                    TableColumn<Regime, String> myColumn = new TableColumn<>(CommonUtil.getMapFieldName(fieldName));
                    myColumn.setCellValueFactory(new PropertyValueFactory<Regime, String>(fieldName));
                    myColumn.setId(fieldName);
                    mainTable.getColumns().add(myColumn);
                //}
    ...
    private void deleteRegimes(ObservableList<Regime> selectedEntity) {
    ...
    private void loadDataInTableView() {
        ...
        if (dataList.size() > 0) {
            if (currentEntity == null) {
                ...
                Integer index = getIndexByRegime(currentEntity);
                if (index >= 0) {
    ...
    void setColumnProperty() {
        ...
        if (createDate != null) {
            ...
            createDate.setCellFactory(column -> {
                TableCell<Regime, String> cell = new TableCell<>() {
    ...
    private void showEditCreateWindow(EnumAction action) {
        ...
        try {
            ...
            Stage RegimeDialogAction = new Stage();
            RegimeDialogAction.setTitle("Создание/редактирование");

            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(Main.class.getResource("/fxml/dictionary/RegimeEditForm.fxml"));

            AnchorPane RegimeAction = loader.load();

            RegimeAction.setId("dictionaryFormAction");

            // Bind controller and main app
            RegimeEditFormController controller = loader.getController();
            controller.setMainApp(mainApp);
            controller.setStage(RegimeDialogAction);
            ...
            RegimeDialogAction.setScene(new Scene(RegimeAction));
            RegimeDialogAction.initOwner(mainApp.getPrimaryStage());
            RegimeDialogAction.initModality(Modality.APPLICATION_MODAL);
            RegimeDialogAction.showAndWait();
