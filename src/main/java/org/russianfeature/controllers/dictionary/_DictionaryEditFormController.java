...
public class _DictionaryEditFormController<T> {
    ...
    public void addSelectButtonEventHandler(Button selectBtn, String fieldTypeName) {
        selectBtn.setOnAction(event -> {
            try {
                ...
                loader.setLocation(Main.class.getResource("/fxml/dictionary/_DictionaryListForm.fxml"));
                ...
                _DictionaryListFormController controller = loader.getController();
...
//    }
    ...
    private ObservableList<T> getDoublesGroupDOOList() {
        ...
        Node groupName = dictionaryMainEdit.lookup("#groupDOO");
        if (groupName == null)
            return null;
        ...
        if (action == EnumAction.UPDATE)
            params.put("id", group.getId().toString());
        else
            params.put("id", "0");
